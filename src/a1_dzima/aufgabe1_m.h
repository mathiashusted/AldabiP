...
protected:
  ...
  */
  ...
  //  ...
  std::vector<uint32_t> vec_lookup_table; //таблица шифтов
  std::string my_pat; //сохраняем пэттэрн
  uint32_t size_of_pat; // размер пэттэрна
  uint32_t size_of_table; // размер вектора с шифтами
  uint32_t new_max_value;
};
